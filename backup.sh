...
# #################################################################################
DB_FILE="/usr/local/var/backup/jracd-db.sql.gz"

if [ -f "$DB_FILE" ] ; then
	rm -f "$DB_FILE"
fi
/usr/bin/mysqldump -uroot -h127.0.0.1 --add-locks=false --lock-tables=false --events=true jracd | /bin/gzip > $DB_FILE


# #################################################################################
UPLOAD_FILE_BASE_PATH="/usr/local/var/upload"
UPLOAD_FILE_DIR="jracd"
UPLOAD_BACKUP_FILE="/usr/local/var/backup/jracd-file.tar"
